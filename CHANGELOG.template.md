...
- **Pipeline de sub-agentes seniores** (divide a responsabilidade que antes era monolítica): `tech-planner` (planeia, read-only, `opus`), `laravel-backend` (implementa, com skills pré-carregadas), `code-reviewer` (revê o diff vs CLAUDE.md + segurança, read-only, `opus`) e `adr-author` (regista decisões significativas, só escreve em `docs/adr/`). Partilhados entre stacks; encadeados pelo agente principal/`@claude` (sub-agentes não invocam outros sub-agentes — limitação do Claude Code). Pipeline documentado no `CLAUDE.md` e no system-prompt do `claude.yml`: `spec-writer → tech-planner → implementer → code-reviewer → adr-author`
- **Skills** (`.claude/skills/`) — conhecimento sénior reutilizável, pré-carregado pelos agentes: `laravel-api-feature`, `pest-testing`, `eloquent-performance`, `writing-adrs`, `code-review-rubric`. Política e allow-list em `config/harness/skills.yml` — skills externas (skills.laravel.cloud / skills.sh) só entram vetadas, revistas e fixadas (são supply-chain: prompt de terceiros)
