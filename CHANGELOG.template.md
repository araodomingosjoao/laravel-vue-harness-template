...
- **Skills** (`.claude/skills/`) — conhecimento sénior reutilizável, pré-carregado pelos agentes. Backend: `laravel-api-feature`, `pest-testing`, `eloquent-performance`. Frontend: `vue-component`, `pinia-store`, `vue-testing`, `ui-ux-design` (fundamentos de UX: os 4 estados loading/empty/error/success, WCAG AA, UX de formulários, feedback). Partilhadas: `writing-adrs`, `code-review-rubric`. Política e allow-list em `config/harness/skills.yml` — skills externas só entram vetadas, revistas e fixadas (são supply-chain: prompt de terceiros)
- **Primeira skill externa vetada**: `frontend-design` da **Anthropic** (Apache-2.0, first-party → risco supply-chain ~nulo) vendorada em `.claude/skills/frontend-design/` (com `LICENSE.txt`) para a parte **estética** — complementa a nossa `ui-ux-design` (que cobre os fundamentos de UX que a frontend-design não cobre). Fixada no commit em `config/harness/skills.yml`

### Mudou
- **Idioma do feedback dividido por audiência**: a conversa do PR que um humano lê passa a **português** — comentários do AI review (`ai-review`), veredicto do `code-reviewer`, e tudo o que o `@claude` publica (descrição do PR, spec, perguntas em aberto). Mantém-se em **inglês** o que é mecânico/tooling: mensagens de CI (`::notice::`/`::error::`), output dos scripts e mensagens de commit. Aplicado em `agent-pr.yml`, `claude.yml`, agente `code-reviewer`, skill `code-review-rubric` e `CLAUDE.md`
